...
ToxProgressBar.prototype.create = function () {
  ...
  this.element.innerHTML =
    ...
    this.size +
    'px;">' +
    '   <div class="radial-outer" style="background-color: #ef7bfe; width: ' +
    this.size +
    "px; height: " +
    this.size +
    'px"></div>' +
    '   <div class="radial-mask-1" style="border-top-left-radius: ' +
    (this.size / 2 + 2) +
    ...
    (this.size + 2) +
    'px;left:-1px;"></div>' +
    '   <div class="radial-mask-2" style="border-top-left-radius: ' +
    (this.size / 2 + 2) +
    "px; border-top-right-radius: " +
    (this.size / 2 + 2) +
    "px; background-color: " +
    ...
    'px;left:-1px;"></div>' +
    '   <div class="radial-mask-3" style="visibility: hidden; border-top-left-radius: ' +
    (this.size / 2 + 2) +
    "px; border-top-right-radius: " +
    (this.size / 2 + 2) +
    "px; background-color: #ef7bfe; width: " +
    this.size +
    "px; height: " +
    this.size / 2 +
    "px;top: -" +
    (this.size * 2 + 4) +
    'px;"></div>' +
    '   <div class="radial-inner" style="background-color: #ef7bfe"; width: "' +
    (this.size - this.thickness * 2) +
    "px; height: " +
    (this.size - this.thickness * 2 + 2) +
    "px; top: -" +
    (this.size * 2.5 - this.thickness + 6) +
    "px; left: " +
    this.thickness +
    'px"></div>' +
    "</div>" +
    (content
      ? '<div class="' +
        content.className +
        '" data-vcenter="' +
        content.dataset.vcenter +
        '" style="' +
        (content.dataset.vcenter === "true"
...
ToxProgressBar.prototype.animate = function () {
  ...
  if (progress > 50) {
    element.querySelector(".radial-mask-1").style.transition =
      "transform " + speed / 2 + "ms ease-in";
    element.querySelector(".radial-mask-2").style.transition =
      "transform " + speed * ((progress - 50) / 100) + "ms ease-out";
  } else {
    element.querySelector(".radial-mask-1").style.transition =
      "transform " + speed * (progress / 100) + "ms ease-in";
  }

  setTimeout(function () {
    if (progress > 50) {
      element.querySelector(".radial-mask-1").style.transform =
        "rotate(270deg)";
      setTimeout(function () {
        // more then half round
        element.querySelector(".radial-mask-1").style.visibility = "hidden";
        element.querySelector(".radial-mask-2").style.transform =
          "rotate(" + (90 + progress * 3.6) + "deg)";
        element.querySelector(".radial-mask-3").style.visibility = "visible";
        if (+progress === 100) {
          setTimeout(function () {
            // full, hide mask to prevent white stripes.
